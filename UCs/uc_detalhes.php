...
// AJUSTE 4: aliases para bater com os nomes reais das colunas (sufixo "_uc")
$stmt = $pdo->prepare("
    SELECT
        uc.id,
        uc.nome_uc,
...
$stmtCom = $pdo->prepare("
    SELECT
        id,
        titulo_comunicacao,
        descricao_comunicacao,
        email_comunicante_comunicacao,
        data_hora_envio_comunicacao,
        status_comunicacao
    FROM comunicacao
    WHERE unidade_conservacao_id = :id
    ORDER BY data_hora_envio_comunicacao DESC
");
                    ...
                    <ul class="lista-municipios">
                        <?php foreach ($municipios as $m): ?>
                            <li>
                                ...
                                — <?= htmlspecialchars($m['uf_municipio']) ?>
                            </li>
                        <?php endforeach; ?>
                    </ul>
                <?php else: ?>
    ...
    <section class="comunicacoes">
        ...
        <?php else: ?>
            <?php foreach ($comunicacoes as $com): ?>
                <?php
                    $statusInfo = $statusLabels[$com['status_comunicacao']]
                        ?? ['label' => 'Desconhecido', 'class' => ''];
                ?>
                <div class="card-comunicacao">
                    <div class="com-header">
                        <h4><?= htmlspecialchars($com['titulo_comunicacao']) ?></h4>
                    ...
                    <p class="com-descricao">
                        <?= nl2br(htmlspecialchars($com['descricao_comunicacao'])) ?>
                    </p>

                    <div class="com-footer">
                        <span>
                            📧 <?= htmlspecialchars($com['email_comunicante_comunicacao']) ?>
                        </span>
                        <span>
                            🕐 <?= date(
                                'd/m/Y \à\s H:i',
                                strtotime($com['data_hora_envio_comunicacao'])
                            ) ?>
                        </span>
                    </div>
                </div>
